...
class DB():
    ...
    def unpack(self, r):
        ...
        f5 = list(struct.unpack('{}f'.format(int(len(r[7])/4)), r[7]))  # vit
        return (r[0], r[1], r[2], f1, f2, f3, f4, f5)
    ...
    # return [(id filename filepath list list list list list)]
    def select_all(self):
    ...
    # 传入特征需转化为list类型
    def insert(self, id, filename, filepath, vgg16feat, lbpfeat, colorfeat, glcmfeat, vitfeat):
        ...
        buf5 = struct.pack('%sf' % len(vitfeat), *vitfeat)
        # _binary %s 便于写入blob字段
        sql = "INSERT INTO DATA_VECTOR(FILENAME, FILEPATH, VGG16FEAT, LBPFEAT, COLORFEAT, GLCMFEAT, VITFEAT) \
            VALUES (%s, %s, _binary %s, _binary %s, _binary %s, _binary %s, _binary %s)"

        cursor = self.db.cursor()    
        try:
            # 执行sql语句
            cursor.execute(sql, (filename, filepath, buf1, buf2, buf3, buf4, buf5))
            # 提交到数据库执行
            self.db.commit()
